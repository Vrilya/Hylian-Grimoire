...
public sealed class CharacterProfileStoreStorageTests
{
    ...
    [Fact]
    public void LoadReportsWarningAndUsesDefaultsForMalformedConfig()
    {
        using TempDirectory tempDirectory = TempDirectory.Create();
        CharacterProfileStoreStorage storage = CharacterProfileStoreStorage.CreateIsolated(tempDirectory.Path);
        Directory.CreateDirectory(storage.ConfigDirectory);
        File.WriteAllText(storage.ConfigPath, "{ not json");

        CharacterProfileStore store = CharacterProfileStore.Load(storage);

        Assert.Contains("Character profiles could not be loaded:", store.LoadWarning);
        Assert.Equal(CharacterProfileStore.AutomaticProfileName, store.AutomaticProfileNameSetting);
        Assert.Equal([CharacterProfileStore.DefaultProfileName], store.ProfileNames);
    }
