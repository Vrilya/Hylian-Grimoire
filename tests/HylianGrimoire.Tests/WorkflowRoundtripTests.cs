...
public sealed class WorkflowRoundtripTests
{
    ...
    [Fact]
    public void SelectedHeaderImportDoesNotFallBackWhenSelectedSlotIsMissing()
    {
        using TestDirectory testDir = TestDirectory.Create();
        string headerPath = testDir.PathFor("selected-missing.h");
        List<MessageEntry> originalNes = CreateSingleEntry("original NES");
        List<MessageEntry> originalGer = CreateSingleEntry("original GER");
        List<MessageEntry> originalFra = CreateSingleEntry("original FRA");
        RomMessageData romData = CreateRomData([originalNes, originalGer, originalFra], activeBankIndex: 0);
        var workflow = new HeaderDocumentWorkflow();

        WriteModernWesternHeader(
            headerPath,
            MissingHeaderSlot,
            TextHeaderSlot("GER import"),
            TextHeaderSlot("FRA import"));
        HeaderRomImportResult result = workflow.ImportIntoRom(
            headerPath,
            romData,
            romData.Entries,
        ...
        Assert.Empty(result.ReplacementBanks);
        AssertEntriesMatch(originalNes, result.RomData.Entries);
        ...
        AssertEntriesMatch(originalGer, gerBank.Entries);
        AssertEntriesMatch(originalFra, fraBank.Entries);
    }

    [Fact]
    public void AllWesternHeaderImportPatchesOnlyAvailableWesternBanks()
    {
        using TestDirectory testDir = TestDirectory.Create();
        string headerPath = testDir.PathFor("all-western-partial.h");
        List<MessageEntry> originalNes = CreateSingleEntry("original NES");
        List<MessageEntry> originalGer = CreateSingleEntry("original GER");
        List<MessageEntry> originalFra = CreateSingleEntry("original FRA");
        List<MessageEntry> importedGer = CreateSingleEntry("GER import");
        List<MessageEntry> importedFra = CreateSingleEntry("FRA import");
        RomMessageData romData = CreateRomData([originalNes, originalGer, originalFra], activeBankIndex: 0);
        var workflow = new HeaderDocumentWorkflow();
        ...
        WriteModernWesternHeader(
            headerPath,
            MissingHeaderSlot,
            TextHeaderSlot(importedGer[0].Text),
            TextHeaderSlot(importedFra[0].Text));
        HeaderRomImportResult result = workflow.ImportIntoRom(
            headerPath,
            romData,
            romData.Entries,
        ...
        Assert.Equal([1, 2], result.ReplacementBanks.Keys.Order().ToArray());
        AssertEntriesMatch(originalNes, result.RomData.Entries);
    ...
    [Fact]
    public void HeaderWorkflowImportsTextThatContainsMissingOrUnused()
    {
        using TestDirectory testDir = TestDirectory.Create();
        string headerPath = testDir.PathFor("missing-words.h");
        List<MessageEntry> originalNes = CreateSingleEntry("original NES");
        List<MessageEntry> originalGer = CreateSingleEntry("original GER");
        List<MessageEntry> originalFra = CreateSingleEntry("original FRA");
        List<MessageEntry> importedNes = CreateSingleEntry("You are missing a key");
        List<MessageEntry> importedGer = CreateSingleEntry("Unused room");
        RomMessageData romData = CreateRomData([originalNes, originalGer, originalFra], activeBankIndex: 0);
        var workflow = new HeaderDocumentWorkflow();

        WriteModernWesternHeader(
            headerPath,
            TextHeaderSlot(importedNes[0].Text),
            TextHeaderSlot(importedGer[0].Text),
            MissingHeaderSlot);

        Assert.Equal(
            [CHeaderMessageSlot.Nes, CHeaderMessageSlot.Ger],
            workflow.GetAvailableWesternImportSlots(headerPath));
        ...
        Assert.Equal([0, 1], result.ReplacementBanks.Keys.Order().ToArray());
        AssertEntriesMatch(importedNes, result.RomData.Entries);
    ...
    private static List<MessageEntry> CreateSingleEntry(string text)
    {
        return
        [
            new MessageEntry(0x6004, 0, 0, 7, 0)
    ...
    private const string MissingHeaderSlot = "MSG(/* MISSING */)";

    private static string TextHeaderSlot(string text) => $"MSG(\"{text}\")";

    private static void WriteModernWesternHeader(string path, string nesSlot, string gerSlot, string fraSlot)
    {
