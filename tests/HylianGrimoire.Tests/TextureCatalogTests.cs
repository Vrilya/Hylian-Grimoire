...
public sealed class TextureCatalogTests
{
    ...
    [Fact]
    public void Texture_export_name_uses_catalog_output_name()
    {
        RomVersionProfile profile = GetProfile("Retail NTSC 1.2");

        TextureDefinition texture = TextureCatalog.GetTextures(profile).Single(texture => texture.Name == "gQuestIconMagicJarBigTex");

        Assert.Equal("big_magic_jar_icon", texture.OutputName);
        Assert.Equal("big_magic_jar_icon", texture.ExportName);
    }

    [Fact]
    public void Texture_export_name_falls_back_to_definition_name()
    {
        var texture = new TextureDefinition(
            "group",
            "gTextureName",
            RomAddress: 0,
            Width: 8,
        ...
        Assert.Equal("gTextureName", texture.ExportName);
    }
    ...
    [InlineData("Majora's Mask NTSC-U", 100)]
    public void Supported_profiles_expose_item_name_texture_targets(string profileName, int expectedCount)
    {
        RomVersionProfile profile = GetProfile(profileName);
    ...
    [Theory]
    [InlineData("Retail NTSC 1.0", 12)]
    [InlineData("Retail PAL 1.0", 36)]
    [InlineData("Retail PAL Master Quest", 36)]
    [InlineData("Retail PAL GameCube", 36)]
    [InlineData("NTSC 1.0", 12)]
    [InlineData("PAL 1.0", 12)]
    [InlineData("NTSC iQue", 12)]
    public void Ocarina_profiles_expose_map_name_point_texture_targets(string profileName, int expectedCount)
    {
        RomVersionProfile profile = GetProfile(profileName);

        IReadOnlyList<TextureDefinition> textures = MapNameTextureCatalog.GetTargets(profile);

        Assert.Equal(expectedCount, textures.Count);
        Assert.All(textures, texture =>
        {
            Assert.True(MapNameTextureCatalog.IsMapNameTexture(texture));
            Assert.Contains("PointName", texture.Name, StringComparison.Ordinal);
            Assert.DoesNotContain("PositionName", texture.Name, StringComparison.Ordinal);
            Assert.Equal("textures/map_name_static", texture.Group);
            Assert.Equal(128, texture.Width);
            Assert.Equal(16, texture.Height);
            Assert.Equal(TextureFormat.IA4, texture.Format);
            Assert.Equal(TextureStorageKind.Rom, texture.StorageKind);
        });
        Assert.DoesNotContain(textures, texture => texture.Name.EndsWith("JPNTex", StringComparison.Ordinal));
    }

    [Theory]
    [InlineData("Retail NTSC 1.0", 22)]
    [InlineData("Retail PAL 1.0", 66)]
    [InlineData("Retail PAL Master Quest", 66)]
    [InlineData("Retail PAL GameCube", 66)]
    [InlineData("NTSC 1.0", 22)]
    [InlineData("PAL 1.0", 22)]
    [InlineData("NTSC iQue", 22)]
    public void Ocarina_profiles_expose_map_position_name_texture_targets(string profileName, int expectedCount)
    {
        RomVersionProfile profile = GetProfile(profileName);

        IReadOnlyList<TextureDefinition> textures = MapPositionNameTextureCatalog.GetTargets(profile);

        Assert.Equal(expectedCount, textures.Count);
        Assert.All(textures, texture =>
        {
            Assert.True(MapPositionNameTextureCatalog.IsMapPositionNameTexture(texture));
            Assert.Contains("PositionName", texture.Name, StringComparison.Ordinal);
            Assert.DoesNotContain("PointName", texture.Name, StringComparison.Ordinal);
            Assert.Equal("textures/map_name_static", texture.Group);
            Assert.Equal(80, texture.Width);
    ...
    [Fact]
    public void Majoras_mask_text_texture_editor_catalog_support_is_limited_to_supported_targets()
    {
        RomVersionProfile profile = GetProfile("Majora's Mask NTSC-U");

        Assert.True(ItemNameTextureCatalog.TryGetTargets(profile, out IReadOnlyList<TextureDefinition>? itemNames));
        Assert.Equal(100, itemNames.Count);
        Assert.True(MajorasMaskMapNameTextureCatalog.TryGetTargets(profile, out IReadOnlyList<TextureDefinition>? mapNames));
        Assert.Equal(16, mapNames.Count);
        Assert.True(MajorasMaskDungeonMapNameTextureCatalog.TryGetTargets(profile, out IReadOnlyList<TextureDefinition>? dungeonMapNames));
        Assert.Equal(4, dungeonMapNames.Count);
        Assert.True(MajorasMaskGameOverTextureCatalog.TryGetTargets(profile, out IReadOnlyList<GameOverTextureTarget>? gameOverTargets));
        Assert.Equal(2, gameOverTargets.Count);
        Assert.True(MajorasMaskPausePromptTextureCatalog.TryGetTargets(profile, out IReadOnlyList<TextureDefinition>? pausePrompts));
        Assert.Equal(7, pausePrompts.Count);
        Assert.False(MapNameTextureCatalog.TryGetTargets(profile, out _));
        Assert.False(MapPositionNameTextureCatalog.TryGetTargets(profile, out _));
        Assert.False(PausePromptTextureCatalog.TryGetTargets(profile, out _));
        Assert.False(DungeonMapNameTextureCatalog.TryGetTargets(profile, out _));
        Assert.False(FileSelectTextureCatalog.TryGetTargets(profile, out _));
        Assert.False(EndTitleTextureCatalog.TryGetTargets(profile, out _));
        Assert.False(PlaceTitleCardTextureCatalog.TryGetTargets(profile, out _));
        Assert.False(BossTitleCardTextureCatalog.TryGetTargets(profile, out _));
        Assert.False(GameOverTextureCatalog.TryGetTargets(profile, out _));
        Assert.False(PauseHeaderTextureCatalog.TryGetTargets(profile, out _));
    }

    [Fact]
    public void Majoras_mask_profile_exposes_map_name_texture_targets()
    {
        RomVersionProfile profile = GetProfile("Majora's Mask NTSC-U");

        IReadOnlyList<TextureDefinition> textures = MajorasMaskMapNameTextureCatalog.GetTargets(profile);

        Assert.Equal(16, textures.Count);
        Assert.All(textures, texture =>
        {
            Assert.True(MajorasMaskMapNameTextureCatalog.IsMapNameTexture(texture));
            Assert.Equal(TextureStorageKind.CmpDmaArchive, texture.StorageKind);
        });
    }

    [Fact]
    public void Majoras_mask_profile_exposes_pause_prompt_texture_targets()
    {
        RomVersionProfile profile = GetProfile("Majora's Mask NTSC-U");

        IReadOnlyList<TextureDefinition> textures = MajorasMaskPausePromptTextureCatalog.GetTargets(profile);

        Assert.Equal(
            [
                "gPauseToEquipENGTex",
                "gPauseToPlayMelodyENGTex",
                "gPauseToViewNotebookENGTex",
                "gPauseToSelectItemENGTex",
                "gPauseToMapENGTex",
                "gPauseToQuestStatusENGTex",
                "gPauseToMasksENGTex",
            ],
            textures.Select(texture => texture.Name));
        Assert.All(textures, texture =>
        {
            Assert.True(MajorasMaskPausePromptTextureCatalog.IsPausePromptTexture(texture));
            Assert.Equal("interface/icon_item_jpn_static", texture.Group);
            Assert.Equal(TextureFormat.IA4, texture.Format);
            Assert.Equal(TextureStorageKind.Rom, texture.StorageKind);
        });
    }
    ...
    [Fact]
    public void Majoras_mask_profile_exposes_dungeon_map_name_texture_targets()
    {
        RomVersionProfile profile = GetProfile("Majora's Mask NTSC-U");

        IReadOnlyList<TextureDefinition> textures = MajorasMaskDungeonMapNameTextureCatalog.GetTargets(profile);

        Assert.Equal(
            [
                "gPauseWoodfallTitleENGTex",
                "gPauseSnowheadTitleENGTex",
                "gPauseGreatBayTitleENGTex",
                "gPauseStoneTowerTitleENGTex",
            ],
            textures.Select(texture => texture.Name));
        Assert.All(textures, texture =>
        {
            Assert.True(MajorasMaskDungeonMapNameTextureCatalog.IsDungeonMapNameTexture(texture));
            Assert.Equal("interface/icon_item_jpn_static", texture.Group);
            Assert.Equal(128, texture.Width);
            Assert.Equal(16, texture.Height);
            Assert.Equal(TextureFormat.IA8, texture.Format);
    ...
    [InlineData("gItemNameAllNightMaskENGTex", "All-Night Mask")]
    [InlineData("gItemNameBombBag20ENGTex", "Bomb Bag (Holds 20)")]
    [InlineData("gItemNameBombBag30ENGTex", "Bomb Bag (Holds 30)")]
    [InlineData("gItemNameBombBag40ENGTex", "Bomb Bag (Holds 40)")]
    [InlineData("gItemNameBombersNotebookENGTex", "Bombers' Notebook")]
    [InlineData("gItemNameDonGerosMaskENGTex", "Don Gero's Mask")]
    [InlineData("gItemNameElegyOfEmptynessENGTex", "Elegy of Emptiness")]
    [InlineData("gItemNameFierceDeitysMaskENGTex", "Fierce Deity's Mask")]
    [InlineData("gItemNameFireArrowENGTex", "Fire Arrow (MP2)")]
    [InlineData("gItemNameFullMilkENGTex", "Milk")]
    [InlineData("gItemNameGreatFairysMaskENGTex", "Great Fairy's Mask")]
    [InlineData("gItemNameHalfMilkENGTex", "Milk (1/2)")]
    [InlineData("gItemNameHerosBowENGTex", "Hero's Bow")]
    [InlineData("gItemNameIceArrowENGTex", "Ice Arrow (MP2)")]
    [InlineData("gItemNameLightArrowENGTex", "Light Arrow (MP4)")]
    [InlineData("gItemNameQuiver30ENGTex", "Quiver (Holds 30)")]
    [InlineData("gItemNameQuiver40ENGTex", "Quiver (Holds 40)")]
    [InlineData("gItemNameQuiver50ENGTex", "Quiver (Holds 50)")]
    ...
    [Theory]
    [InlineData("gMapPointClockTownENGTex", "Clock Town")]
    [InlineData("gMapPointGreatBayCoastENGTex", "Great Bay Coast")]
    [InlineData("gMapPointIkanaGraveyardENGTex", "Ikana Graveyard")]
    [InlineData("gMapPointZoraHallENGTex", "Zora Hall")]
    public void Majoras_mask_map_name_texture_names_can_be_converted_to_display_text(
        string textureName,
        string expectedText)
    {
        string displayText = MajorasMaskMapNameTextureCatalog.GetDisplayText(textureName);

        Assert.Equal(expectedText, displayText);
    }

    [Theory]
    [InlineData("gDeathMountainPointNameENGTex", "Death Mountain", "English")]
    [InlineData("gGerudosFortressPointNameFRATex", "Gerudo's Fortress", "French")]
    [InlineData("gHyliaLakesidePointNameENGTex", "Lake Hylia", "English")]
    [InlineData("gHyruleFieldPointNameENGTex", "Hyrule Field", "English")]
    [InlineData("gKakarikoVillagePointNameGERTex", "Kakariko Village", "German")]
    [InlineData("gZorasDomainPointNameENGTex", "Zora's Domain", "English")]
    public void Ocarina_map_name_point_texture_names_expose_display_text_and_language(
        string textureName,
        string expectedText,
        string expectedLanguage)
    {
        string displayText = MapNameTextureCatalog.GetDisplayText(textureName);

        Assert.Equal(expectedText, displayText);
        Assert.Equal(expectedLanguage, MapNameTextureCatalog.GetLanguage(new TextureDefinition(
    ...
    [Theory]
    [InlineData("gDeathMountainCraterPositionNameENGTex", "Death Mountain\r\nCrater", "English")]
    [InlineData("gDeathMountainTrailPositionNameENGTex", "Death Mountain\r\nTrail", "English")]
    [InlineData("gDesertColossusPositionNameENGTex", "Desert\r\nColossus", "English")]
    [InlineData("gGanonsCastlePositionNameENGTex", "Ganon's\r\nCastle", "English")]
    [InlineData("gGerudoValleyPositionNameENGTex", "Gerudo\r\nValley", "English")]
    [InlineData("gGerudosFortressPositionNameFRATex", "Gerudo's\r\nFortress", "French")]
    [InlineData("gGoronCityPositionNameENGTex", "Goron City", "English")]
    [InlineData("gGraveyardPositionNameENGTex", "Graveyard", "English")]
    [InlineData("gHauntedWastelandPositionNameENGTex", "Haunted\r\nWasteland", "English")]
    [InlineData("gHyruleCastlePositionNameENGTex", "Hyrule\r\nCastle", "English")]
    [InlineData("gHyruleFieldPositionNameENGTex", "Hyrule Field", "English")]
    [InlineData("gKakarikoVillagePositionNameENGTex", "Kakariko\r\nVillage", "English")]
    [InlineData("gKokiriForestPositionNameENGTex", "Kokiri Forest", "English")]
    [InlineData("gLakeHyliaPositionNameENGTex", "Lake Hylia", "English")]
    [InlineData("gLonLonRanchPositionNameENGTex", "Lon Lon\r\nRanch", "English")]
    [InlineData("gLostWoodsPositionNameENGTex", "Lost Woods", "English")]
    [InlineData("gMarketPositionNameENGTex", "Market", "English")]
    [InlineData("gQuestionMarkPositionNameENGTex", "?", "English")]
    [InlineData("gSacredForestMeadowPositionNameGERTex", "Sacred Forest\r\nMeadow", "German")]
    [InlineData("gZorasDomainPositionNameENGTex", "Zora's\r\nDomain", "English")]
    [InlineData("gZorasFountainPositionNameENGTex", "Zora's\r\nFountain", "English")]
    [InlineData("gZorasRiverPositionNameENGTex", "Zora's River", "English")]
    public void Ocarina_map_position_name_texture_names_expose_display_text_and_language(
        string textureName,
        string expectedText,
        string expectedLanguage)
    {
        string displayText = MapPositionNameTextureCatalog.GetDisplayText(textureName);

        Assert.Equal(expectedText, displayText);
        Assert.Equal(expectedLanguage, MapPositionNameTextureCatalog.GetLanguage(new TextureDefinition(
            "textures/map_name_static",
            textureName,
            0,
            80,
    ...
    [Theory]
    [InlineData("gPauseToEquipENGTex", "to Equip", "English")]
    [InlineData("gPauseToPlayMelodyENGTex", "to Play Melody", "English")]
    [InlineData("gPauseToSelectItemENGTex", "To Select Item", "English")]
    [InlineData("gPauseToMapENGTex", "To Map", "English")]
    [InlineData("gPauseToQuestStatusENGTex", "To Quest Status", "English")]
    [InlineData("gPauseToMasksENGTex", "To Masks", "English")]
    [InlineData("gPauseToViewNotebookENGTex", "to View Notebook", "English")]
    public void Majoras_mask_pause_prompt_texture_names_expose_display_text_and_language(
        string textureName,
        string expectedText,
        string expectedLanguage)
    {
        TextureDefinition texture = MajorasMaskPausePromptTextureCatalog.GetTargets(GetProfile("Majora's Mask NTSC-U"))
            .Single(texture => texture.Name == textureName);

        Assert.Equal(expectedText, MajorasMaskPausePromptTextureCatalog.GetDisplayText(texture));
        Assert.Equal(expectedLanguage, MajorasMaskPausePromptTextureCatalog.GetLanguage(texture));
    }

    [Fact]
    public void Item_name_renderer_fit_to_width_only_scales_horizontal_axis_when_font_available()
    {
        ...
        const string text = "Quiver (Holds 50) Bomb Bag (Holds 40)";
        var settings = new ItemNameTextureRenderSettings(
            FontSize: 12,
            StrokeWidth: 2.75,
            XNudge: 0,
            BaselineY: 12,
        ...
        using Bitmap unfit = ItemNameTextureRenderer.Render(
            text,
            fontPath,
            settings with { FitToWidth = false },
            256,
            ItemNameTextureCatalog.Height);
        using Bitmap fitted = ItemNameTextureRenderer.Render(
            text,
            fontPath,
            settings with { FitToWidth = true },
            ItemNameTextureCatalog.Width,
            ItemNameTextureCatalog.Height);

        Rectangle unfitBounds = GetAlphaBounds(unfit);
        Rectangle fittedBounds = GetAlphaBounds(fitted);

        Assert.False(unfitBounds.IsEmpty);
        Assert.False(fittedBounds.IsEmpty);
        Assert.True(fittedBounds.Width < unfitBounds.Width);
        Assert.InRange(Math.Abs(fittedBounds.Height - unfitBounds.Height), 0, 1);
    }

    [Theory]
    [InlineData("gPauseWoodfallTitleENGTex", "Woodfall Temple", "English")]
    [InlineData("gPauseSnowheadTitleENGTex", "Snowhead Temple", "English")]
    [InlineData("gPauseGreatBayTitleENGTex", "Great Bay Temple", "English")]
    [InlineData("gPauseStoneTowerTitleENGTex", "Stone Tower Temple", "English")]
    public void Majoras_mask_dungeon_map_name_texture_names_expose_display_text_and_language(
        string textureName,
        string expectedText,
        string expectedLanguage)
    {
        TextureDefinition texture = MajorasMaskDungeonMapNameTextureCatalog.GetTargets(GetProfile("Majora's Mask NTSC-U"))
            .Single(texture => texture.Name == textureName);

        Assert.Equal(expectedText, MajorasMaskDungeonMapNameTextureCatalog.GetDisplayText(texture));
        Assert.Equal(expectedLanguage, MajorasMaskDungeonMapNameTextureCatalog.GetLanguage(texture));
    }
    ...
    [Fact]
    public void Majoras_mask_profile_exposes_complete_game_over_texture_targets()
    {
        RomVersionProfile profile = GetProfile("Majora's Mask NTSC-U");

        IReadOnlyList<GameOverTextureTarget> targets = MajorasMaskGameOverTextureCatalog.GetTargets(profile);

        Assert.Equal(2, targets.Count);
        GameOverTextureTarget target = targets.Single(target => target.Spec.Kind == GameOverTextureTargetKind.GameOver);
        Assert.Equal("GAME OVER", target.Spec.SampleText);
        Assert.Equal(["gGameOverP1Tex", "gGameOverP2Tex", "gGameOverP3Tex"], target.Textures.Select(texture => texture.Name));
        Assert.All(target.Textures, texture =>
        {
            Assert.True(MajorasMaskGameOverTextureCatalog.IsGameOverTripletTexture(texture));
            Assert.Equal("interface/icon_item_gameover_static", texture.Group);
            Assert.Equal(TextureStorageKind.Rom, texture.StorageKind);
        });

        GameOverTextureTarget continuePlaying = targets.Single(target => target.Spec.Kind == GameOverTextureTargetKind.ContinuePlaying);
        Assert.Equal("Continue playing ?", continuePlaying.Spec.SampleText);
        Assert.Equal(["gContinuePlayingNESTex"], continuePlaying.Textures.Select(texture => texture.Name));
        Assert.True(MajorasMaskGameOverTextureCatalog.IsContinuePlayingTexture(continuePlaying.Texture));
        Assert.False(continuePlaying.Texture.Name.Contains("JAP", StringComparison.Ordinal));
    }
    ...
    [Fact]
    public void Compact_text_renderer_can_tuck_text_after_wide_capitals_without_moving_left_edge_when_font_available()
    {
        string fontPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Fonts), "arial.ttf");
        if (!File.Exists(fontPath))
        {
        ...
        using Bitmap normal = CompactTextTextureRenderer.Render("TA", fontPath, settings, 96, 18);
        using Bitmap tucked = CompactTextTextureRenderer.Render(
            "TA",
            fontPath,
            settings with { CapitalTRightTuck = 2.5 },
            96,
            18);

        Rectangle normalBounds = GetAlphaBounds(normal);
        Rectangle tuckedBounds = GetAlphaBounds(tucked);

        Assert.Equal(normalBounds.Left, tuckedBounds.Left);
        Assert.True(tuckedBounds.Width < normalBounds.Width);

        using Bitmap normalWide = CompactTextTextureRenderer.Render("WA", fontPath, settings, 96, 18);
        using Bitmap tuckedWide = CompactTextTextureRenderer.Render(
            "WA",
            fontPath,
            settings with { CapitalWRightTuck = 2.5 },
            96,
            18);

        Rectangle normalWideBounds = GetAlphaBounds(normalWide);
        Rectangle tuckedWideBounds = GetAlphaBounds(tuckedWide);

        Assert.Equal(normalWideBounds.Left, tuckedWideBounds.Left);
        Assert.True(tuckedWideBounds.Width < normalWideBounds.Width);
    }
