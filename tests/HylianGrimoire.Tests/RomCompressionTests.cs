...
public sealed class RomCompressionTests
{
    ...
    [Fact]
    public void FontOrderApplyEditorTextReportsUnencodableCharacters()
    {
        var entry = new MessageEntry(FontOrderCodec.MessageId, type: 0, position: 0, bank: 7, offset: 0);

        FontOrderUpdateResult result = FontOrderService.ApplyEditorText(entry, "ABC☃");

        Assert.False(result.Changed);
        Assert.Contains("cannot be encoded", result.ErrorMessage);
        Assert.Null(entry.EncodedBytesOverride);
    ...
    public void SaveMessagesUpdatesChecksumForUncompressedMessagePatchesWhenAvailable()
    {
        string fixturePath = LocalRomFixtures.GetRequiredRetailDecompressedPath("oot_retail_ntsc_1.2_decompressed.z64");
        MessageBankProfile bank = new("Messages", 0x22000, 0x0100, 0x20000, 0x0010);
        var profile = new RomVersionProfile(
            "Checksum Save Test",
            string.Empty,
            BuildDateOffset: 0,
            DmaTableOffset: 0x1800,
            DmaEntryCount: 2,
            RomCodecKind.Yaz0,
            ...
            [bank],
            new HashSet<int>());
        byte[] rom = File.ReadAllBytes(fixturePath);
        WriteDmaEntryAt(
            rom,
            profile.DmaTableOffset,
            virtualStart: (uint)bank.MessageDataOffset,
            virtualEnd: (uint)(bank.MessageDataOffset + bank.MessageDataSize),
            physicalStart: (uint)bank.MessageDataOffset,
            physicalEnd: 0);
        WriteDmaEntryAt(
            rom,
            profile.DmaTableOffset + 16,
            virtualStart: (uint)(bank.MessageDataOffset + 0x0100),
            virtualEnd: (uint)(bank.MessageDataOffset + 0x0200),
            physicalStart: (uint)(bank.MessageDataOffset + 0x0100),
            physicalEnd: 0);
        Assert.True(N64Checksum.TryUpdate(rom));
        ...
        try
        {
            RomMessageService.SaveMessages(tempPath, source, entries, compressOverride: false);

            byte[] saved = File.ReadAllBytes(tempPath);
            AssertChecksumMatchesRom(saved);
            Assert.True(ReadUInt32BigEndian(saved, profile.DmaTableOffset + 4) > bank.MessageDataOffset + bank.MessageDataSize);
        }
        finally
        {
            File.Delete(tempPath);
    ...
    private static void AssertChecksumMatchesRom(byte[] rom)
    {
        byte[] expected = rom.ToArray();

        Assert.True(N64Checksum.TryUpdate(expected));
        Assert.Equal(expected.AsSpan(0x10, 8).ToArray(), rom.AsSpan(0x10, 8).ToArray());
    }
    ...
    private static void WriteDmaEntry(byte[] rom, int index, uint virtualStart, uint virtualEnd, uint physicalStart, uint physicalEnd)
    {
        ...
        WriteDmaEntryAt(rom, offset, virtualStart, virtualEnd, physicalStart, physicalEnd);
    }

    private static void WriteDmaEntryAt(byte[] rom, int offset, uint virtualStart, uint virtualEnd, uint physicalStart, uint physicalEnd)
    {
