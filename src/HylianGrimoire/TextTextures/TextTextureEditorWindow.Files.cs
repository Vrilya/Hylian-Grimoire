...
public sealed partial class TextTextureEditorWindow
{
    private async void OnSavePng(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            PngFileWriter.Save(_lastGenerated, path);
    ...
    private async void OnApplyToRom(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            if (!IsLastGeneratedCurrent(_romData.DecompressedRom, item))
            {
                RefreshPreview();
                const string staleMessage = "The selected ROM texture changed after this preview was generated. Review the refreshed preview, then apply it again.";
                SetStatus(staleMessage);
                await ShowErrorAsync("Preview is out of date", staleMessage);
                return;
            }
    ...
    private string CreateSuggestedFileName()
    {
        string text = _selectedTextureKind switch
        {
            ...
            TextTextureKind.MapPositionNames => $"{MapPositionLine1TextBox.Text.Trim()} {MapPositionLine2TextBox.Text.Trim()}".Trim(),
