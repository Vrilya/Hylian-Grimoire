...
{
    private const string CacheVersion = "prompt-editor-v9";
        ...
        bool showFrames)
    {
        ...
        using Bitmap panelLeft = ReadAsset(rom, profile.IconSegmentRomBase, profile.IconAssets["PanelLeft"]);
        using Bitmap panelRight = ReadAsset(rom, profile.IconSegmentRomBase, profile.IconAssets["PanelRight"]);
        ...
        try
        {
            for (int i = 0; i < lines.Count; i++)
            {
                ...
                using Bitmap icon = ReadAsset(rom, profile.IconSegmentRomBase, profile.IconAssets[line.IconKey]);
                using Bitmap text = ReadAsset(rom, GetTextBase(profile, languageKey), profile.TextAssets[languageKey][line.TextKey]);
    ...
    private static int GetTextBase(PromptEditorProfile profile, string languageKey) => languageKey switch
    {
        "ger" => profile.TextGermanRomBase ?? profile.TextEnglishRomBase,
        "fra" => profile.TextFrenchRomBase ?? profile.TextEnglishRomBase,
        _ => profile.TextEnglishRomBase,
    };

    private static Bitmap ReadAsset(ReadOnlySpan<byte> rom, int segmentBase, PromptEditorAsset asset)
    {
        int offset = segmentBase + asset.LocalOffset;
        int length = asset.Width * asset.Height;
        if (offset < 0 || offset + length > rom.Length)
        {
            throw new InvalidDataException($"Pause-prompt asset extends past EOF at 0x{offset:x8}.");
        }

        Color color = asset.ColorDisplayListOffset is int dlistOffset
            ? ReadDisplayListPrimColor(rom, segmentBase + dlistOffset)
            : asset.FallbackColor;
        return Ia8ToBitmap(rom.Slice(offset, length), asset.Width, asset.Height, color);
    }

    private static Color ReadDisplayListPrimColor(ReadOnlySpan<byte> rom, int offset)
    {
        int end = Math.Min(offset + 0x80, rom.Length);
        for (int cursor = offset; cursor + 7 < end; cursor += 8)
        {
            if (rom[cursor] == 0xfa)
            {
                return Color.FromArgb(rom[cursor + 7], rom[cursor + 4], rom[cursor + 5], rom[cursor + 6]);
            }
        }
    ...
    private static Bitmap Ia8ToBitmap(ReadOnlySpan<byte> data, int width, int height, Color color)
    {
        ...
        IReadOnlyList<PromptEditorLine> lines)
    {
        ...
        for (int i = 0; i < lines.Count; i++)
        {
            ...
            Rectangle textRect = new(N64ToPixel(line.TextX), y, Scale(textAsset.Width, scale), Scale(textAsset.Height, scale));
        ...
        bool showFrames)
    {
        ...
        AppendAssetBytes(hash, rom, profile.IconSegmentRomBase, profile.IconAssets.Values);
        foreach (string key in profile.LanguageKeys)
        {
            ...
            {
                AppendAssetBytes(hash, rom, GetTextBase(profile, key), assets.Values);
            }
        }
    ...
    private static void AppendAssetBytes(IncrementalHash hash, ReadOnlySpan<byte> rom, int baseOffset, IEnumerable<PromptEditorAsset> assets)
    {
        foreach (PromptEditorAsset asset in assets)
        {
            int offset = baseOffset + asset.LocalOffset;
            int length = asset.Width * asset.Height;
            if (offset >= 0 && offset + length <= rom.Length)
            {
