...
public static partial class CompactTextTextureRenderer
{
        ...
        int height)
    {
        ...
        float capitalTRightTuck = Math.Max(0, (float)(NormalizeCharacterSpacing(settings.CapitalTRightTuck) * scale));
        float capitalWRightTuck = Math.Max(0, (float)(NormalizeCharacterSpacing(settings.CapitalWRightTuck) * scale));
        bool usesPerElementLayout = characterSpacing != 0 || capitalTRightTuck != 0 || capitalWRightTuck != 0;
        int textElementCount = usesPerElementLayout ? CountTextElements(runs) : 0;
        ...
        foreach (DrawingTextRun run in runs)
        {
            ...
            else
            {
                ...
                foreach (string textElement in EnumerateTextElements(run.Text))
                {
                    ...
                    runAdvance -= GetCapitalRightTuck(capitalTRightTuck, capitalWRightTuck, textElement, textElementIndex, textElementCount);
                    textElementIndex++;
                }
