...
public static class TextureCatalog
{
    ...
    private static readonly IReadOnlyDictionary<string, string> CatalogFileByProfileName = new Dictionary<string, string>(StringComparer.Ordinal)
    {
        ...
        ["NTSC 1.0"] = "swedish_ntsc_10.txt",
        ["NTSC 1.1"] = "swedish_ntsc_11.txt",
        ["NTSC 1.2"] = "swedish_ntsc_12.txt",
        ["NTSC Master Quest"] = "swedish_ntsc_mq.txt",
        ["NTSC GameCube"] = "swedish_ntsc_gc.txt",
        ["PAL 1.0"] = "swedish_pal_10.txt",
        ["PAL 1.1"] = "swedish_pal_11.txt",
        ["PAL Master Quest"] = "swedish_pal_mq.txt",
        ["PAL GameCube"] = "swedish_pal_gc.txt",
        ["NTSC iQue"] = "swedish_ique.txt",
        ["PAL iQue"] = "swedish_ique.txt",
        ["NTSC MQ iQue"] = "swedish_ique_mq.txt",
        ["PAL MQ iQue"] = "swedish_ique_mq.txt",
        ["Majora's Mask NTSC-U"] = "mm_us_n64.txt",
    };
    ...
    public static bool TryGetTextures(RomVersionProfile profile, out IReadOnlyList<TextureDefinition> textures)
    {
        ...
        string path = GetCatalogPath(profile, fileName);
        if (!File.Exists(path))
        {
            textures = [];
            return false;
    ...
    private static string GetCatalogPath(RomVersionProfile profile, string fileName)
        => Path.Combine(AppContext.BaseDirectory, profile.GameProfile.Assets.TextureCatalogRoot, fileName);
    ...
    {
        if (parts.Length is not 7 and not 9 and not 10 and not 12)
        {
        ...
        if (usesTlut && parts.Length is not 9 and not 12)
        {
            throw new InvalidDataException($"Texture definition in {fileName} line {lineNumber} uses {format} but has no TLUT fields.");
        }

        int storageIndex = usesTlut ? 9 : 7;
        TextureStorageKind storageKind = TextureStorageKind.Rom;
        int? archiveRomAddress = null;
        int? archiveLength = null;
        if (parts.Length > storageIndex)
        {
            if (parts.Length != storageIndex + 3)
            {
                throw new InvalidDataException($"Invalid texture storage fields in {fileName} line {lineNumber}.");
            }

            storageKind = ParseStorageKind(parts[storageIndex]);
            archiveRomAddress = int.Parse(parts[storageIndex + 1], NumberStyles.HexNumber, CultureInfo.InvariantCulture);
            archiveLength = int.Parse(parts[storageIndex + 2], NumberStyles.HexNumber, CultureInfo.InvariantCulture);
        }
        ...
        return new TextureDefinition(
            ...
            format,
            usesTlut ? int.Parse(parts[7], NumberStyles.HexNumber, CultureInfo.InvariantCulture) : null,
            usesTlut ? int.Parse(parts[8], CultureInfo.InvariantCulture) : null,
            parts[2],
            storageKind,
            archiveRomAddress,
            archiveLength);
    ...
    private static TextureStorageKind ParseStorageKind(string value)
        => value switch
        {
            "CmpDmaArchive" => TextureStorageKind.CmpDmaArchive,
            "Rom" => TextureStorageKind.Rom,
            _ => throw new InvalidDataException($"Unsupported texture storage kind: {value}."),
        };
}
