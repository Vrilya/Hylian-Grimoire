...
public static class TextureRomService
{
    public static byte[] ReadRaw(ReadOnlySpan<byte> rom, TextureDefinition texture)
    {
        ...
        if (texture.UsesArchive)
        {
            byte[] archive = ReadArchive(rom, texture);
            ValidateRange(archive.Length, texture.RomAddress, length, texture.Name);
            return archive.AsSpan(texture.RomAddress, length).ToArray();
        }
    ...
    public static byte[] ReadTlutRaw(ReadOnlySpan<byte> rom, TextureDefinition texture)
    {
        ...
        {
            byte[] archive = ReadArchive(rom, texture);
            ValidateRange(archive.Length, tlutAddress, length, $"{texture.Name} TLUT");
            return archive.AsSpan(tlutAddress, length).ToArray();
        }
    ...
    public static void WriteRaw(Span<byte> rom, TextureDefinition texture, ReadOnlySpan<byte> data)
    {
        ...
        if (texture.UsesArchive)
        {
            if (texture.ArchiveRomAddress is not { } archiveRomAddress || texture.ArchiveLength is not { } archiveLength)
            {
                throw new InvalidDataException($"Texture {texture.Name} uses a CmpDma archive but has no archive location.");
            }

            CmpDmaArchive.WriteSlice(rom, archiveRomAddress, archiveLength, texture.RomAddress, data);
            return;
        }
    ...
    private static byte[] ReadArchive(ReadOnlySpan<byte> rom, TextureDefinition texture)
    {
        if (texture.ArchiveRomAddress is not { } archiveRomAddress || texture.ArchiveLength is not { } archiveLength)
        {
            throw new InvalidDataException($"Texture {texture.Name} uses a CmpDma archive but has no archive location.");
        ...
        return CmpDmaArchive.DecodeAll(rom, archiveRomAddress, archiveLength);
    }
}
