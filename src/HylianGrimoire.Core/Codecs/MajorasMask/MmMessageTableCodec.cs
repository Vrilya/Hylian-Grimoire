...
public static class MmMessageTableCodec
{
        ...
        int? tableSegment = null)
    {
        ...
        {
            EncodedMessageTableEntry tableEntry = EncodedMessageTableEntry.Read(tableBytes.AsSpan(i, EntrySize));
            i += EntrySize;

            if (tableEntry.Id == EndMarkerId)
            {
                finalEndMarkerBank = tableEntry.Bank;
                finalEndMarkerOffset = tableEntry.Offset;
                break;
            }

            int bank = tableEntry.Bank;
            int offset = tableEntry.Offset;
            if (bank != expectedTableSegment || offset < 0 || offset >= messageBytes.Length)
            {
                throw new InvalidDataException($"Message 0x{tableEntry.Id:x4} has an invalid Majora's Mask message pointer.");
            }

            MajorasMaskMessageMetadata metadata = ReadMetadata(messageBytes, offset, tableEntry.TypePosition);
            var entry = new MessageEntry(tableEntry.Id, metadata.Type, metadata.Position, bank, offset)
            {
    ...
    private static int FindTableStart(byte[] tableBytes, int? tableSegment = null)
    {
        ...
        {
            EncodedMessageTableEntry entry = EncodedMessageTableEntry.Read(tableBytes.AsSpan(i, EntrySize));
            int bank = entry.Bank;
    ...
    private static void WriteEntry(List<byte> output, int id, byte typePosition, int bank, int offset)
        => EncodedMessageTableEntry.FromTypePosition(id, typePosition, bank, offset).WriteTo(output);
}
