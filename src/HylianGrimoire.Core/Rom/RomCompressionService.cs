...
{
    public static RomCompressionResult DecompressRom(
        ...
        IProgress<RomFileOperationProgress>? progress = null)
    {
        ...
        progress?.Report(new RomFileOperationProgress(completedFiles, totalFiles));
    ...
    private static void WriteCompressedDmaTable(Span<byte> output, RomVersionProfile profile, IReadOnlyList<PackedDmaEntry> packedEntries)
    {
        ...
        foreach (PackedDmaEntry packed in packedEntries.OrderBy(entry => entry.Entry.Index))
        {
            DmaEntry entry = packed.Entry;
            int offset = profile.DmaTableOffset + (entry.Index * 16);
            ...
            if (entry.IsDeleted || entry.IsEmpty)
            {
                DmaTable.WriteUInt32BigEndian(output, offset + 8, entry.PhysicalStart);
                DmaTable.WriteUInt32BigEndian(output, offset + 12, entry.PhysicalEnd);
                continue;
            }
