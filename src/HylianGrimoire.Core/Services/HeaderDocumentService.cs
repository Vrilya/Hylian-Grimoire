...
public sealed record HeaderFileDocument(
    Dictionary<int, List<MessageEntry>> Languages,
...
public static class HeaderDocumentService
{
        ...
        GameEncodingProfileResolver? getEncodingProfile = null)
    {
        ...
        return new HeaderFileDocument(
            ...
            LoadJapaneseEntriesFromContent(content, getEncodingProfile));
    }
    ...
    {
        string content = File.ReadAllText(path);
        List<MessageEntry> parsedEntries = TryImportHeaderSlot(content, slot, allowWesternFallback: false, getEncodingProfile);
    ...
    private static IReadOnlyList<MessageEntry>? LoadJapaneseEntriesFromContent(
        string content,
        GameEncodingProfileResolver? getEncodingProfile)
    {
        List<MessageEntry> entries = TryImportHeaderSlot(content, CHeaderMessageSlot.Jpn, allowWesternFallback: false, getEncodingProfile);
